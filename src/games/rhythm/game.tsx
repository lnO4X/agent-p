...
export default function RhythmGame({
  ...
  const gameLoop = useCallback(() => {
    ...
    ctx.textAlign = "left";
    ctx.fillText(`命中: ${deviationsRef.current.length}`, 10, 25);
    ctx.textAlign = "right";
    ctx.fillText(`剩余: ${TOTAL_NOTES - processedRef.current}`, CANVAS_W - 10, 25);

    ctx.textAlign = "center";
    ctx.fillStyle = "rgba(255,255,255,0.4)";
    ctx.font = "12px sans-serif";
    ctx.fillText("按 空格键 或 点击", CANVAS_W / 2, CANVAS_H - 15);
  ...
  const handleHit = useCallback(() => {
    ...
    if (bestNote) {
      ...
      if (bestDev < 30) setLastHitText("完美!");
      else if (bestDev < 80) setLastHitText("很好!");
      else setLastHitText("还行");
    }
  }, [phase]);
    ...
    <div className="flex flex-col items-center gap-4 w-full">
      ...
      {phase === "idle" && (
        <div className="w-full max-w-sm bg-slate-800 rounded-xl p-6 md:p-8 text-center">
          <p className="text-lg mb-4">节拍捕手</p>
          <p className="text-sm text-muted-foreground mb-6">
            音符从上方三条轨道落下，到达蓝色区域时按空格键或点击画布。
            节奏从90BPM开始逐渐加快。共20个音符，计时偏差越小分数越高。
          </p>
          <button
            onClick={startGame}
      ...
      {phase === "done" && (
        <div className="w-full max-w-sm bg-slate-800 rounded-xl p-6 md:p-8 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-2">平均计时偏差</p>
          <p className="text-sm text-muted-foreground mt-1">
            命中 {hitCount}/{TOTAL_NOTES}
          </p>
        </div>
      )}
